  ...
  ngOnInit(){
    ...
    if (storedStudente) {
      try {
        ...
        this.safeUrl = this.sanitizer.bypassSecurityTrustResourceUrl(this.tipoBase + this.studente.cv);
        console.log(this.studente);
      } catch (e) {
        console.error('Error parsing studente from local storage:', e);
      }
    } else {
      console.log('No studente in local storage');
    }
  }
